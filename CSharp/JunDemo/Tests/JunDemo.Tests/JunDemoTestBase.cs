...
namespace JunDemo.Tests
{
    public abstract class JunDemoTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<JunDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<JunDemoDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JunDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JunDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JunDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JunDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<JunDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JunDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JunDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JunDemoDbContext, Task<T>> func)
        {
            T result;
